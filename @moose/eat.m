...
%eating function for class moose
%agt=moose object
%cn - current agent number
%eaten = 1 if moose finds food, =0 otherwise
...
%SUMMARY OF moose EAT RULE
%moose detects food level in its 1km x 1km square of the environment
%if food> 1, moose will consume food
%otherwise moose food level decremented by 1.
...
if pfood>=1                             %if food exists at this location
    ...
    eaten=1;                             %moose has eaten - set flag to one
else
    agt.food=cfood-1;                   %decrease agent food by one unit
    eaten=0;                            %flag tells moose to migrate
end
