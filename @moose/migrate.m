function [agt]=migrate(agt,cn,agent)
%% Changes in this file
%[agt]=migrate(agt,cn) -> [agt]=migrate(agt,cn,agent)

%%

%migration functions for class moose
%agt=moose object
%cn - current agent number

%SUMMARY OF moose MIGRATE RULE
%mooses will migrate only if they have not eaten
%mooses will always try to migrate towards the nearest food source
%The moose will extract the distibution of food in its LOCAL environment (at
...
mig=0;                               %indicates whether moose has successfully migrated
pos=agt.pos;                         %extract current position 
cpos=round(pos);                     %round up position to nearest grid point   
spd=agt.speed;                       %moose migration speed in units per iteration - this is equal to the food search radius
...
mig=0;                          %flag will be reset to one if moose migrates
...
if mig==0                                   %moose has been unable to find food, so chooses a random direction to move in      
    cnt=1;
    dir=rand*2*pi;              
    while mig==0&cnt<=8                     
        npos(1)=pos(1)+spd*cos(dir);        %new x co-ordinate
        npos(2)=pos(2)+spd*sin(dir);        %new y co-ordinate
        if npos(1)<ENV_DATA.bm_size&npos(2)<ENV_DATA.bm_size&npos(1)>=1&npos(2)>=1   %check that moose has not left edge of model - correct if so.
           mig=1;
        end
        cnt=cnt+1;
