...
nr=IT_STATS.tot_r;
nf=IT_STATS.tot_f;
eaten=IT_STATS.eaten;
disp(strcat('Iteration = ',num2str(N_IT)))
disp(strcat('No. new mooses = ',num2str(IT_STATS.div_r(N_IT+1))))
disp(strcat('No. new wolves = ',num2str(IT_STATS.div_f(N_IT+1))))
disp(strcat('No. agents migrating = ',num2str(IT_STATS.mig(N_IT+1))))
disp(strcat('No. mooses dying = ',num2str(IT_STATS.died_r(N_IT+1))))
disp(strcat('No. wolves dying = ',num2str(IT_STATS.died_f(N_IT+1))))
disp(strcat('No. mooses eaten = ',num2str(IT_STATS.eaten(N_IT+1))))
...
if (fmode==false) || (N_IT==nsteps) || ((fmode==true) && (rem(N_IT , CONTROL_DATA.fmode_display_every)==0))
    ...
    col{1}='r-';                   %set up colours that will represent different cell types red for mooses, blue for wolves
    col{2}='b-';
    col{3}='g-';
    ...
    subplot(3,1,3),cla
    subplot(3,1,3),plot((1:N_IT+1),eaten(1:N_IT+1),col{3});
    subplot(3,1,3),axis([0 nsteps 0 y_max]);
    %subplot(4,1,4),cla
    %subplot(4,1,4),plot((1:N_IT+1),tot_food(1:N_IT+1),'m-');
    %subplot(4,1,4),axis([0 nsteps 0 tot_food(1)]);
    subplot(3,1,1),title('No. live mooses');
    subplot(3,1,2),title('No. live wolves');
    subplot(3,1,3),title('No. eaten mooses');
    %subplot(4,1,4),title('Total food');
    drawnow
